...
#Get missclass rate
avsens=as.data.frame(avclassifyrate(df, ctmod)$Average)
print(avsens)
save(avsens, file="MisclassClassTree.RData")

#Make percentage classification table
pclass=merge(count(rftrain$finpred), count(rftrain$ctpred), by="x", all=T)
pclass=pclass[,-1]
pclass=(pclass/nrow(rftrain))*100
#Calculate differences in classification relative to final model
diffrf=ifelse(rftrain$finpred==rftrain$ctpred,0,1)
...
colnames(pclass)=c("Final Model","Classification Tree")
pclass[is.na(pclass)]=0
print(pclass)
save(pclass, file="Classification_Table_for ClassTree.RData")
